...
public class TaxCategoriesPanel extends JPanelTable2 {

    private JParamsSites m_paramsSite;
    private DataLogicSync dlSync;
    private TaxCategoriesEditor jeditor;
    ...
    protected void init() {
        ...
        m_paramsSite.addActionListener(new TaxCategoriesPanel.ReloadActionListener());
        ...
        lpr = new ListProviderCreator(new PreparedSentence(app.getSession(),
                ...
                new TaxCategoriesPanel.TaxCategoriesSerializerRead()
        ),
                m_paramsSite);
        ...
        SentenceExec updatesent = new SentenceExecTransaction(app.getSession()) {
            ...
            public int execInTransaction(Object params) throws BasicException {
                ...
                if (values[2] == null) {
                    ...
                    return new PreparedSentence(app.getSession(), "INSERT INTO TAXCATEGORIES (ID, NAME, SITEGUID) VALUES (?, ?, ?)", new SerializerWriteBasicExt(row.getDatas(), new int[]{0, 1, 2})).exec(params);
                } else {
                    // UPDATE
                    return new PreparedSentence(app.getSession(), "UPDATE TAXCATEGORIES SET NAME = ? WHERE ID = ? AND SITEGUID = ? ", new SerializerWriteBasicExt(row.getDatas(), new int[]{1, 0, 2})).exec(params);
                }
            }
        };
        ...
        SentenceExec deletesent = new SentenceExecTransaction(app.getSession()) {
            ...
            public int execInTransaction(Object params) throws BasicException {
                Object[] values = (Object[]) params;
                values[2] = m_paramsSite.getSelectKey();
                return new PreparedSentence(app.getSession(), "DELETE FROM TAXCATEGORIES WHERE ID = ? AND SITEGUID = ? ", new SerializerWriteBasicExt(row.getDatas(), new int[]{0, 2})).exec(params);
            }
        };
    ...
    private class ReloadActionListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            try {
                TaxCategoriesPanel.this.bd.actionLoad();
            } catch (BasicException w) {
            }
        }
    }

    private class TaxCategoriesSerializerRead implements SerializerRead {
