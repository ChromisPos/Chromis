...
// Puts the tree into alpabetical order.    
    ...
    public Object createValue() throws BasicException {
        ...
        if (!hasPermissions) {
            ...
            if (JOptionPane.showOptionDialog(this,
                ...
                role[0] = m_oId == null ? UUID.randomUUID().toString() : m_oId;
                role[1] = m_jName.getText();
                role[2] = Formats.BYTEA.parseValue(m_dlAdmin.findRolePermissions(role[0].toString(), siteGuid));
    ...
    public void getPermissions() throws BasicException {
        //Get the permissions assigned to the selected user.
        try {
            // Get the role Id with username and site guid
            String m_roles = m_dlAdmin.getRoleID(m_jName.getText(), siteGuid);
            // The permissions for the user
            String sRolePermisions = m_dlAdmin.findRolePermissions(m_roles, siteGuid);
